...
# 详细字段(dict，可选)：{指标:.., 参数:.., 测试数据:.., 数据来源:..}——回答"用了什么指标/什么参数/
#   基于什么数据测试/数据从哪来"，渲染为「📖 详细说明」块。缺某键自动跳过。
...
CATALOG = [
    # ───────────── 选股策略 · 量化研究栈 ─────────────
    {
        ...
        "详细": {
            "指标": "缠论结构指标——分型 → 笔 → 线段 → 中枢 → 背驰；多级别联立(日线本级别 + 30分钟次级别)。",
            "参数": "买点类型由中枢/背驰逻辑判 1买/2买/3买(chanlun_engine.py)；次级别确认固定 30 分钟；批量扫描股票池近 N 交易日窗口。",
            "测试数据": "全 A 股日 K + 30 分钟 K；每日收盘后 chanlun_batch 预计算并落 chanlun_signals.db(20:00 调度)。",
            "数据来源": "日 K 四级自动降级链：本地通达信(TDX) → AKTools → akshare → Tushare(akshare_gateway/data_source_manager)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "六维多空灯：MACD、KDJ、RSI、LWR、MTM、BBI(红=多头)，看共振红灯数。",
            "参数": "多头数门槛 min_bull=5(≥5 红)；各维红绿灯公式见 liumai_engine.py。",
            "测试数据": "全 A 股日 K；liumai_batch 批量计算入 liumai_signals.db 最新批次。",
            "数据来源": "日 K 四级降级链(本地 TDX → AKTools → akshare → Tushare)，同缠论。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "缠论买点(1/2/3买) + 六脉神剑红灯数，取交集。",
            "参数": "时间窗 ±3 交易日；六脉 ≥5 红。",
            "测试数据": "chanlun_signals.db ∩ liumai_signals.db → combo_signals.db。",
            "数据来源": "两库均基于日 K 四级降级链(本地 TDX 优先)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "核心A=缠论买点+极限抄底+量比+机构净买；核心B=1买+尖刺金叉；卖=相对强弱+六脉红灯+MA20 上行。",
            "参数": "A∪B(量比≥1.3 / 1买+尖刺金叉)；大盘风险 SID≤2；剔除获利盘>70%；移动止盈 +25~30%。",
            "测试数据": "全历史缠论买卖点经 walk-forward 样本外验证(训练≤2023末，样本外 2024~2025.10)，融入全量信号库共性。",
            "数据来源": "缠论/六脉 DB + profit_mining 信号库特征(本地 TDX 全历史日 K)。",
        },
        "关键参数": [
    ...
    # ───────────── 选股策略 · AI智能体栈 ─────────────
    {
        ...
        "详细": {
            "指标": "主力资金净流入排名 + 市值/资金二次筛选。",
            "参数": "取净流入前 100；市值区间 min/max_market_cap；回溯 days_ago / start_date。",
            "测试数据": "实时问财查询快照(选股结果，非历史回测)。",
            "数据来源": "pywencai(同花顺问财)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "MA 均线择时(在 pywencai 选出的低价高成长池上)。",
            "参数": "max_stocks=4 / 个股仓位 0.4 / 持股 5 天 / 单日最大买入 2。",
            "测试数据": "问财低价高成长股池 + 日 K 上的均线买卖信号。",
            "数据来源": "选股池 pywencai(同花顺问财)；择时日 K 走四级降级链。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "基本面门槛：总市值 / 营收增速 / 净利增速。",
            "参数": "市值≤50 亿、营收增速≥10%、净利增速≥100%、沪深 A、非 ST/创业/科创；top_n=5。",
            "测试数据": "实时问财查询快照。",
            "数据来源": "pywencai(同花顺问财)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "基本面：净利润同比增速。",
            "参数": "净利增速≥10%、深圳 A、非科创/创业/ST；按成交额升序；top_n=5。",
            "测试数据": "实时问财查询快照。",
            "数据来源": "pywencai(同花顺问财)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "解释": "价值投资筛选——PE≤20、PB≤1.5、股息率≥1%、资产负债率≤30%、非ST/非科创/非创业，按流通市值由小到大；"
                "配套 RSI 超买 + 持股周期择时策略。",
        "详细": {
            "指标": "估值因子(PE/PB/股息率/资产负债率) + RSI 超买择时。",
            "参数": "PE≤20、PB≤1.5、股息≥1%、负债率≤30%；holding_period=30、rsi_overbought=70。",
            "测试数据": "问财选股池 + akshare 日 K(ak.stock_zh_a_hist)做择时回测。",
            "数据来源": "选股 pywencai(同花顺问财)；择时日 K 用 akshare。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "龙虎榜多维评分(席位/资金/连板) + AI 智能体解读。",
            "参数": "评分维度与权重见 longhubang_scoring.py(席位质量/资金强度/连板等)。",
            "测试数据": "龙虎榜明细数据(分析型，非历史回测)。",
            "数据来源": "第三方龙虎榜 HTTP API(lhb-api.ws4.cn/v1)。",
        },
        "关键参数": [
    ...
    # ───────────── 买入卖出策略 ─────────────
    {
        ...
        "详细": {
            "指标": "买：缠论买点+极限抄底+量比+机构净买 / 1买+尖刺金叉；卖：均线全多头·连板(过热顶)、相对强弱+六脉红灯+MA20 上行(强势顶)。",
            "参数": "量比≥1.3；强势顶相对强弱≥5；移动止盈 +25~30%；胜率口径见星级分档。",
            "测试数据": "全历史缠论买卖点经 walk-forward 样本外验证。",
            "数据来源": "缠论 DB + profit_mining 信号库特征(本地 TDX 全历史日 K)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "全量信号库原子信号(复用 mine_combos 评分器算覆盖率/提升度)。",
            "参数": "好卖点标签=卖后跌≥4%；分组 全部/1卖/2卖/3卖。",
            "测试数据": "缠论卖点事件 + ±窗口信号特征(signal_features.csv / sell_features.csv)。",
            "数据来源": "profit_mining 特征库(本地 TDX 日 K + 全历史缠论卖点 events_labeled.csv)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "稳定组合 A∪B 规则 + 实时价位判定 + 星级排序。",
            "参数": "A∪B + 大盘 SID≤2 + 剔除获利盘>70%；可入状态(可入/尾窗/已过窗/已涨过/已破止损/已止盈)，价格类优先于窗口类。",
            "测试数据": "最近缠论买点 + 当日实时行情判状态。",
            "数据来源": "缠论 DB + 实时行情(四级降级链)。",
        },
        "关键参数": [
    ...
    # ───────────── 测试盈利策略 ─────────────
    {
        ...
        "详细": {
            "指标": "全量信号库特征加权得到的『合成分』 → 映射样本外胜率。",
            "参数": "胜率口径≥4% / 大涨≥10%；训练截止 2023末，样本外 2024~2025.10；各档星级阈值固化于 star_thresholds.json。",
            "测试数据": "全历史 1买 信号(纯标准库回测：合成分 → 未来收益胜率)。",
            "数据来源": "profit_mining 特征库(本地 TDX 全历史日 K + 缠论 1买 事件)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "L1单/L2两两/L3三联 条件组合(全量信号库原子信号)。",
            "参数": "SUP_TRAIN=200 / SUP_TEST=30 / TOPK_TRIPLE=16。",
            "测试数据": "训练段挖规则 → 测试未来段纯验证(滚动切分，量化样本内偏差)。",
            "数据来源": "profit_mining 评分库(signal_features.csv / scores_all_combos.csv)。",
        },
        "关键参数": [
    ...
    # ───────────── 找共同点策略 ─────────────
    {
        ...
        "详细": {
            "指标": "方案A=斐波那契回踩企稳(ratio 0.382/0.5/0.618/0.786) × MACD 金叉；方案B=BBI(cross 上穿 / above 站上) × MACD。",
            "参数": "ZigZag 阈值 6%/10%/15%/20%；正样本=拐点后 [L,L+4] 起涨/起跌初期；MACD fast/slow/signal 网格；覆盖率门槛 0.5、最佳可达取 Top30。",
            "测试数据": "4415 只股票全历史日 K，按 ZigZag 各阈值切涨跌段定正样本，逐股累加信号命中算覆盖率/提升度/精确度。",
            "数据来源": "本地 TDX 全历史日 K(akshare_gw.local.get_kline)；股票池=events 全历史事件 4415 码。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "全量原子信号库的 L1单/L2两两/L3三联/L5 组合。",
            "参数": "盈利覆盖门槛 COVER_MIN=0.70；进榜最小支持 SUPPORT_MIN=50；三联 TOPK_TRIPLE=18。",
            "测试数据": "signal_features.csv 上向量化算每个组合的覆盖率/提升度/胜率并排序。",
            "数据来源": "profit_mining 特征库(本地 TDX 日 K + 全历史缠论买卖点 events_labeled.csv)。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "全量原子信号库(features.py 定义：缠论六脉/经典指标/通达信信号/庄散/波动率等)。",
            "参数": "盈利标签阈值 WIN_THRESH=4.0%；窗口偏移 OFFSET=±2(防泄漏)。",
            "测试数据": "逐股日 K + 大盘指数，对每个信号 ±2 窗口算布尔特征 → signal_features.csv。",
            "数据来源": "本地 TDX 日 K(akshare_gw.local.get_kline) + 大盘指数。",
        },
        "关键参数": [
    ...
    },
    {
        ...
        "详细": {
            "指标": "方案A/B 信号 × 分桶维度(波动率/市值/行业/板块)；看各组 uplift = lift_group − lift_all。",
            "参数": "分桶 cuts(波动率=按事件近20日振幅三分位 / 市值三分位 / 行业 / 板块)；达标共性>50%；组级 seg≥3000、行级 fires≥300。",
            "测试数据": "共性挖掘同一批样本(全历史日 K 切段)按维度分桶，求各组最优参数与全市场 uplift。",
            "数据来源": "本地 TDX 日 K + 市值快照(腾讯 qt.gtimg.cn) + 行业/板块快照(events)。",
        },
        "关键参数": [
