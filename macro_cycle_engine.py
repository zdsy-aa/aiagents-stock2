...
class MacroCycleEngine:
    ...
    def __init__(self, model=None):
        ...
        logger.info(f"[宏观周期引擎] 初始化完成 (模型: {self.model})")
    ...
    def run_full_analysis(self, progress_callback=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not raw_data.get("success"):
                logger.warning("⚠ 数据采集未完全成功，尝试继续分析...")
            ...
            logger.info("✓ 数据采集和格式化完成")
            logger.info(f"  数据文本长度: {len(formatted_text)} 字符")
